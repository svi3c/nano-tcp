...
export class NanoServer<T> {
  #sockets: NanoSocket[] = [];
  #subscriptions: { [topic in number | string]: Set<NanoSocket> } = {};
  #server: Server;
  #messageHandlers: {
      ...
      socket.on("message", async (message: string) => {
        ...
        switch (type) {
          case "+": {
            ...
            const requestId = Number(message.substr(idx1 + 1));
            socket.send(`<${requestId}|0|`);
            break;
          }
          case "-": {
          ...
          case "?": {
            ...
            try {
              ...
              socket.send(`<${requestId}|0|${responsePayload}`);
            } catch (e) {
              socket.send(`<${requestId}|${e.code}|${e.message}`);
            }
          }
        }
      });
